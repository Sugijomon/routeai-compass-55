...
// Filterstate wordt extern beheerd zodat DpoMatrix en UsageFlows dezelfde selectie kunnen gebruiken.
...
import type { ToolFiltersState, QuickFilter } from "./useToolFilters";

interface Props {
  totalCount: number; // alle tools (vóór filter)
  filtered: ToolRow[]; // resultaat ná filter
  state: ToolFiltersState;
  setFilter: <K extends keyof ToolFiltersState>(key: K, value: ToolFiltersState[K]) => void;
  resetFilters: () => void;
...
export function ToolRegisterTable({
  totalCount,
  filtered,
  state,
  setFilter,
  ...
  return (
    <section className="overflow-hidden rounded-[14px] border border-slate-200 bg-white shadow-[0_8px_24px_rgba(26,32,44,.06)]">
      <div className="flex flex-wrap items-start justify-between gap-4 px-6 pt-5 pb-3">
        <div className="flex min-w-0 flex-1 items-start gap-3">
          <ClipboardList className="mt-0.5 shrink-0 text-[#0369a1]" size={24} />
          <div>
            <h3 className="text-lg font-bold">AI-toolregister</h3>
            <p className="mt-0.5 text-[12.5px] text-[#566166]">
        ...
        <span className="rounded-full border border-slate-200 bg-slate-50 px-3 py-1.5 text-[10px] font-bold text-slate-500">
          {sorted.length} van {totalCount} tools
        </span>
      </div>

      {/* Quick filters + search */}
      <div className="flex flex-wrap items-center gap-2 border-b border-slate-100 px-6 pb-3">
        {QUICK_FILTERS.map(({ key, label, icon: Icon }) => {
          const active = state.quick === key;
          return (
            <button
              key={key}
              onClick={() => setFilter("quick", key)}
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left text-[12.5px]">
          <thead className="bg-slate-50/60 text-[10px] uppercase tracking-wide text-slate-500">
            <tr>
              <th className="px-4 py-2.5 font-extrabold">Tool</th>
              <th className="px-4 py-2.5 font-extrabold">
                <select
                  value={state.riskClass}
                  onChange={(e) => setFilter("riskClass", e.target.value)}
              ...
              <th className="px-4 py-2.5 font-extrabold">Top vakgebied</th>
              <th className="px-4 py-2.5 text-right font-extrabold">Meldingen</th>
              <th className="px-4 py-2.5 text-right font-extrabold">Trend</th>
            </tr>
          </thead>
          <tbody>
            {sorted.length === 0 && (
            ...
            {sorted.map((t) => {
              ...
              return (
                ...
                >
                  ...
                  <td className="px-4 py-2.5 text-right font-mono font-extrabold tabular-nums text-slate-700">
                    {t.reports}
                  </td>
                  <td
                    className="px-4 py-2.5 text-right font-mono text-[11px] font-extrabold tabular-nums"
